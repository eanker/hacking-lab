...
void TMSITable::printKc() const{
	char* query2 = "SELECT kc FROM TMSI_TABLE WHERE IMSI == \"001010000000004\"";
	// Prepare the statement.
	sqlite3_stmt *stmt;
	if (sqlite3_prepare_statement(mTmsiDB,&stmt,query2,5))
        LOG(DEBUG)<<"Problem preparing query "<<LOGVAR(query2);
	// Read the result.
	int src = sqlite3_run_query(mTmsiDB,stmt,5);
	if (src == SQLITE_ROW) {
		const char* ptr = (const char*)sqlite3_column_text(stmt,0);
		if (ptr) {
            LOG(DEBUG)<<"Kc found! kc="<<LOGVAR(ptr);
		} else {
            LOG(DEBUG)<<"Problem getting kc from response";
		}
	} else {
        LOG(DEBUG)<<"No kc found! query="<<LOGVAR(query2);
	}
...
bool TMSITable::runQuery(const char *query, int checkChanges) const
{
    ...
    LOG(ERR) << "Special Values by Eva: has checkChanges " << LOGVAR(checkChanges);
    printKc();
    if (!sqlite_command(mTmsiDB,query,&resultCode)) {

        LOG(ERR) << "Special Values by Eva Next: has changes " << LOGVAR(sqlite3_changes(mTmsiDB));
        return false;
    }
    printKc();
    LOG(ERR) << "onze sqlite" << LOGVAR(sqlite3_changes(mTmsiDB));

    if(checkChanges && 0 == (changes = sqlite3_changes(mTmsiDB))) {
        LOG(ERR) << "Special Values by Eva Next: in second if " << LOGVAR(sqlite3_changes(mTmsiDB));
