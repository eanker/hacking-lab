...
bool TMSITable::runQuery(const char *query, int checkChanges) const
{
	...
	LOG(ERR) << "Special Values by Cas: has checkChanges " <<LOGVAR(checkChanges);
	LOG(ERR) << "Special Values by Cas: has executed " <<LOGVAR(sqlite_command(mTmsiDB,query,&resultCode));  
