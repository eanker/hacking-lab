	...
	if (authorization != AuthUnauthorized) {
		...
		switch (ludata()->getTmsiStatus()) {
			case tmsiFailed:
				...
				// Fall through...
				LOG(ERR) <<"L3State: tmsiFailed "<< ludata()->getTmsiStatus();
			case tmsiNone: {
				...
				LOG(ERR) <<"L3State: tmsiNone "<< ludata()->getTmsiStatus();
			...
			case tmsiNotAssigned: {
				...
				LOG(ERR) <<"L3State: tmsiFNotAssigned "<< ludata()->getTmsiStatus();
			...
			case tmsiProvisional:	// The TMSI from the tmsi table authenticated.
				LOG(ERR) <<"L3State: tmsiProvisional "<< ludata()->getTmsiStatus();
			...
			case tmsiAuthenticated:
				LOG(ERR) <<"L3State: tmsiAuthenticated "<< ludata()->getTmsiStatus();
			case tmsiNew:
				// These cases should not occur here.
				LOG(ERR) <<"L3State: tmsiNew "<< ludata()->getTmsiStatus();
				devassert(0);
